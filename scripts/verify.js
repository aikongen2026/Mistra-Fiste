...
for(const file of ['public/index.html','public/app.js','public/model.js','public/river-points.js','public/style.css','public/sw.js','public/data/lures.json','server.js'])if(!fs.existsSync(path.join(root,file)))throw Error('Mangler '+file);
...
for(const file of ['server.js','public/app.js','public/model.js','public/river-points.js','public/sw.js'])cp.execFileSync(process.execPath,['--check',path.join(root,file)],{stdio:'pipe'});
